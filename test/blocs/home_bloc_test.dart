...
void main(){
  ...
  group("Home Bloc Test", () {
    ...
    test("Fetch Now Playing Movies Test",(){
      expect(homeBloc?.mNowPlayingMoviesList?.contains(getMockMoviesForTest().first), true);
    });

    test("Fetch Popular Movies Test",(){
      expect(homeBloc?.mPopularMoviesList?.contains(getMockMoviesForTest()[1]), true);
    });


    test("Fetch Top Rated Movies Test",(){
      expect(homeBloc?.mTopRatedMoviesList?.contains(getMockMoviesForTest().last), true);
    });
    ...
    test("Fetch Initial Movies By Genres Test",() async{
      ...
      expect(homeBloc?.mMoviesByGenreList?.contains(getMockMoviesForTest().first), true);
    });
    ...
    test("Fetch Movies By Genres From User",() async{

      homeBloc?.onTapGenre(3);

      await Future.delayed(Duration(milliseconds: 500));

      expect(homeBloc?.mMoviesByGenreList?.contains(getMockMoviesForTest().last), true);
    });
